...
def get_rankings():
    ...
    response = requests.get(
        ...
        params={"year": year, "seasonType": "both", "division": "fbs"},
    ...
    data = data[data['home_division']== 'fbs']  # Adjusted field name based on API docs; confirm if 'home_classification' in your response
    ...
    # convert each game into two rows, one for each team and remove unwanted rows.
    teams = pd.concat([
    ...
    #list of teams
    teams3 = list(teams2.team.unique())  # Use unique to avoid duplicates and save memory
    # create empty arrays
    ts2 = []
    vics2 = []
    loss2 = []
    ...
    for ind,row in schedule2.iterrows():
        ...
        per = len(row['Victories']) / (len(row['Victories']) + len(row['Losses'])) if (len(row['Victories']) + len(row['Losses'])) > 0 else 0  
        records.append(rec)
        win_p.append(per)
    ...
    # add rating points for victories over top 25 teams
    # top 25 = 1
    # top 10 = 2
    # top 5 = 3
    ...
    url = "http://www.collegepollarchive.com/football/ap/seasons.cfm?seasonid=" + str(year)
    ...
    # loop to attach ap rankings to all ranked schools
    ap = pd.DataFrame()
    rank = []
    college = []
    for i in range(1,26):
...
def get_rankings():
    ...
    PR = pd.DataFrame(ratings, columns=['team', 'adj'])
    # create spread variable
    data2['homeSpread'] = data2['homePoints'] - data2['awayPoints']
    data2['awaySpread'] = -data2['homeSpread']


    # convert each game into two rows, one for each team and remove unwated rows.
    teams2 = pd.concat([
    data2[['homeTeam', 'homeSpread', 'awayTeam', ]].rename(columns={'homeTeam': 'team', 'homeSpread': 'spread',  'awayTeam': 'opponent'}),
    data2[['awayTeam', 'awaySpread', 'homeTeam' ,]].rename(columns={'awayTeam': 'team', 'awaySpread': 'spread',  'homeTeam': 'opponent'})
    ])
